...
function ProfessorProfile() {
  ...
  useEffect(() => {
    ...
    const savedBio = localStorage.getItem(`professor_bio_${user.id}`);
    const savedAbout = localStorage.getItem(`professor_about_${user.id}`);
    const savedLikes = JSON.parse(localStorage.getItem(`professor_likes_${user.id}`) || "{}");
    const savedLikesCount = JSON.parse(localStorage.getItem(`professor_likesCount_${user.id}`) || "{}");
    const savedComments = JSON.parse(localStorage.getItem(`professor_comments_${user.id}`) || "{}");
    const savedPosts = JSON.parse(localStorage.getItem(`professor_posts_${user.id}`) || "[]");
  ...
  const saveBio = () => {
    localStorage.setItem(`professor_bio_${currentUser.id}`, bio);
    setEditingBio(false);
  };

  const saveAbout = () => {
    localStorage.setItem(`professor_about_${currentUser.id}`, about);
  ...
  const toggleLike = (postId) => {
    ...
    setLikes(updated);
    localStorage.setItem(`professor_likes_${currentUser.id}`, JSON.stringify(updated));
    // update counts
    setLikesCount((prev) => {
      ...
      localStorage.setItem(`professor_likesCount_${currentUser.id}`, JSON.stringify(updatedCounts));
      ...
      <Container className="mt-4 mb-5">
        <Card className="mb-4 p-3">
          <Row className="align-items-center">
            ...
            <Col>
              ...
              <div className="text-muted mb-2">{currentUser.department || "Professor"}</div>
        ...
        <Card className="p-3 mb-4">
          <div className="d-flex align-items-center mb-2">
            ...
            <Form.Control
              className="ms-3"
              placeholder={`What's on your mind, Prof. ${currentUser.name.split(" ")[0]}?`}
              value={postText}
